...
void KeyFrameDatabase::updateVisualization()
{
    ...
    for (it = keyFrameList.begin(); it != keyFrameList.end(); it++) //遍历关键帧序列
	{
		...
		*/
        ...
        if ((*it)->update_loop_info) //符合条件的关键帧,构建了两条边
		{
			
            KeyFrame* connected_KF = getKeyframe((*it)->loop_index); //关键帧的一个数据成员为loop_index,代表该关键帧对应的闭环帧的index
			Vector3d conncected_P;
			Matrix3d connected_R;
			connected_KF->getPose(conncected_P, connected_R);
            posegraph_visualization->add_loopedge(P, conncected_P); //构建图优化的边
		...
		pose_stamped.header = odometry.header;
		pose_stamped.pose = odometry.pose.pose;
        refine_path.header = odometry.header; //保存path
		refine_path.poses.push_back(pose_stamped);
