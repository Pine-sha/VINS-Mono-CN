...
void readParameters(ros::NodeHandle &n)
{
    ...
    cv::FileStorage fsSettings(config_file, cv::FileStorage::READ); //cv库读取配置文件到一个 map 中
    ...
    fsSettings["image_topic"] >> IMAGE_TOPIC; //读取某一项到 全局变量中
    ...
    LOOP_CLOSURE = fsSettings["loop_closure"]; //读取loop_closure参数值
    if (LOOP_CLOSURE == 1)
    {
