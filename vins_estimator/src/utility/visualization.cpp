                ...
                Eigen::Matrix3d loop_correct_r)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        odometry.pose.pose.position.x = estimator.Ps[WINDOW_SIZE].x(); //世界坐标系下,平移量
        odometry.pose.pose.position.y = estimator.Ps[WINDOW_SIZE].y();
        odometry.pose.pose.position.z = estimator.Ps[WINDOW_SIZE].z();
        odometry.pose.pose.orientation.x = Quaterniond(estimator.Rs[WINDOW_SIZE]).x();//世界坐标系,旋转量
        ...
        pub_path.publish(path); //"path_no_loop"
        ...
        pub_loop_path.publish(loop_path); //"path"
